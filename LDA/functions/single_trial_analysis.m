function [Azloo,Azsig,Y,a,v,D,Azloomedian,Azsig_thr] = single_trial_analysis(X, truth, duration, skipLOO, perm, eigratio, gamma, method)
...
if ~method
    ...
    %[v,D] = logistpca(X',truth_aug,[],1,[],eigratio); % tb edit to show plot
else
...
if ~skipLOO
    ...
    for azsigi=1:azsigimax,
        ...
        for i=1:N
            ...
            if ~method
                ...
                [vloo] = logistpca(tmp,tmpt,[],[],[],eigratio);
            else
                % run lda to get weight vector
    ...
    if perm(1),
        psig = perm(3);
        tmpaz = sort(Azsig,'descend');
        Azsig_thr = tmpaz(round(nperms*psig));
    else
        Azsig = -1;
        Azsig_thr = -1;
    end
    
    % TB edit - don't cut values
